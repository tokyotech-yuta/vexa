...
<Note>
**Community-contributed and unwitnessed.** The contract above is verified against Vexa's own STT
client. This example is not: no Vexa maintainer has run Vexa against a FunASR or SenseVoice gateway,
so the language-quality claim below is the contributor's, not a tested guarantee. Independent
validation is tracked in [#863](https://github.com/Vexa-ai/vexa/issues/863) — reports welcome.
</Note>
